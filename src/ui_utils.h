...
static i32 ui_text_index_from_screen_x(ui_widget_t* widget, ui_text_edit_t* text_edit, ui_text_line_t* text_line, f32 screen_x)
{
    ...
    else
    {
        ...
        {
            f32 char_width = global_ui->font->get_text_width(widget->font.font, text_edit->text + i, 1);
...
static void ui_widget_text_edit_selection(ui_widget_t* widget, ui_text_edit_t* text_edit)
{
    if (text_edit->selection != text_edit->cursor)
    {
        ...
        for (ui_text_line_t* text_line = widget->text_line_list.first; text_line; text_line = text_line->next)
        {
            ...
            f32 start_measure_x = font->get_text_width(widget->font.font, text_edit->text + text_line->offset, selection_start - text_line->offset);
            f32 end_measure_x = font->get_text_width(widget->font.font, text_edit->text + text_line->offset, selection_end - text_line->offset);
...
static void ui_text_edit_update_scroll(ui_widget_t* widget, ui_text_edit_t* text_edit)
{
    ...
    {
        font_t* font = global_ui->font;
        f32 cursor_x = font->get_text_width(widget->font.font, text_edit->text, text_edit->cursor);
        f32 text_width = font->get_text_width(widget->font.font, text_edit->text, text_edit->length);
...
static void ui_widget_text_edit_cursor(ui_widget_t* widget, ui_text_edit_t* text_edit)
{
    ...
    ui_push_parent(widget);
    {
        ...
        {
            font_t* font = global_ui->font;
            cursor_x = text_line->position[UI_AXIS_X] + font->get_text_width(widget->font.font, text_edit->text + text_line->offset, text_edit->cursor - text_line->offset);
...
static void ui_widget_slider(const char* widget_name, const char* label,
                             ui_size_t slider_height, ui_size_t thumb_height, vec4 color,
                             f32* default_value, f32 min_value, f32 max_value)
{
    ...
    ui_widget_row()
    {
        ...
        ui_widget_column()
        {
            ...
            ui_next_size(ui_percent(1.0f, 0.0f), slider_height);
            ui_widget_t* slider_widget = ui_widget_build_from_format_string("%s-slider", widget_name);

            ui_push_parent(slider_widget);
            {
                f32 thumb_width = 16.0f;
                // f32 thumb_height = 16.0f;
                ui_signal_t thumb_signal = ui_widget_last_signal("%s-slider-thumb", widget_name);
                ...
                *default_value = clamped_value;
                ...
                ui_next_size(ui_pixel(thumb_width, 1.0f), thumb_height);
                ui_widget_build_from_format_string("%s-slider-thumb", widget_name);
            }
            ui_pop_parent();
        ...
        ui_widget_t* slider_value_widget = ui_widget_build_from_format_string("%s-slider-value", widget_name);
